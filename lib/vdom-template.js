...
module.exports = function vdomTemplate (rawTemplate, baseIndent) {
  ...
  function createVNodeHS (node, noIndent) {
    ...
    const existElseIf = checkIfExistDirective(node.attrs, 'else-if');
    ...
    if (existEach) {
      ...
      // 需要检测 n-else 前面是否存在 n-if 如果不存在就报错
  ...
  // 需要检测 n-else-if 前面是否存在 n-if
  function createHyperScript (fragment) {
    let result = '[\n';
    indent++;
    for (let idx = 0; idx < fragment.childNodes.length; ++idx) {
      const node = fragment.childNodes[idx]
      if (node.nodeName === '#comment') { // 忽略注释节点
        continue
      }
      if (idx !== 0) {
        result += ',\n'
