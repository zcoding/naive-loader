...
function objectStringify(obj) {
  ...
  for (let p in obj) {
    ...
    if (/^:/.test(p)) {
      ...
      if (/^\s*{[^{}]*}\s*$/.test(value)) {
        const newValue = value.replace(/^\s*{|}\s*$/g, '').split(',');
        const newObject = [];
        for (let i = 0; i < newValue.length; ++i) {
          const keyValue = newValue[i].split(':');
          newObject.push(`${keyValue[0]}:${parseExpression(keyValue[1], combineAlias(aliasStask))}`);
        }
        attrs.push(`"${p}":{${newObject.join(',')}}`);
      } else {
...
module.exports = function vdomTemplate (rawTemplate, baseIndent) {
  ...
  function createVNodeHS (node, noIndent) {
    ...
    if (existEach) {
      ...
      // 处理 each 指令
      const alias = exps[0].trim();
      const scopeVar = exps[1].trim();
      aliasStask.push([alias, '$index']);
      result += writeLine(`_.each(${parseExpression(scopeVar, combineAlias(aliasStask))}, function(${alias}, $index, $key) { return {key: ${_uuid} + "-" + $key, tagName: "${node.tagName}", attrs: ${objectStringify(attrs2props(node.attrs))}, children: `);
    } else if (existIf) {
      // 处理 if 指令
