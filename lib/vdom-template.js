    ...
    fragment.childNodes.forEach((node, idx) => {
      ...
      if (node.nodeName === '#text') {
        ...
        if (existEach) {
          ...
          // 处理 each 指令
          result += writeLine(`_.each(${exps[1].trim()}, function(${exps[0].trim()}, index) { return {key: "${_uuid}-" + index, tagName: "${node.tagName}", attrs: ${objectStringify(attrs2props(node.attrs))}, children: `);
        } else if (existIf) {
          // 处理 if 指令
          conditionStack.push(existIf.value);
          result += writeLine(`_.if(${existIf.value}, {tagName: "${node.tagName}", attrs: ${objectStringify(attrs2props(node.attrs))}, children: `);
        } else if (existElse) {
          const lastCondition = conditionStack.pop();
          result += writeLine(`_.if(!${lastCondition}, {tagName: "${node.tagName}", attrs: ${objectStringify(attrs2props(node.attrs))}, children: `);
        } else {
