...
function handleText (textNode) {
  ...
  _splits.forEach(_text => {
    ...
    if (_parts.length > 1) {
      _textNodes.push(`_._(state, "${_parts[0].replace(/^\s+|\s+$/g, '').replace(/"/g, '\\"')}")`);
      if (_parts[1]) {
...
module.exports = function vdomTemplate (rawTemplate, baseIndent) {
    ...
    fragment.childNodes.forEach((node, idx) => {
      ...
      if (node.nodeName === '#text') {
        ...
        if (existEach) {
          ...
          // 处理 each 指令
          result += writeLine(`_.each("${exps[0].trim()}", "${exps[1].trim()}", state, h, {tagName: "${node.tagName}", attrs: ${JSON.stringify(attrs2props(node.attrs))}, children: `);
        } else if (existIf) {
          // 处理 if 指令
          result += writeLine(`_.if("${existIf.value}", state, h, {tagName: "${node.tagName}", attrs: ${JSON.stringify(attrs2props(node.attrs))}, children: `);
        } else {
          result += writeLine(`h("${node.tagName}", ${JSON.stringify(attrs2props(node.attrs))}, `);
        }
        if (node.childNodes.length > 0) {
          result += createHyperScript(node);
          result += existEach || existIf ? '})' : `, "${_uuid}")`;
        } else {
          result += existEach || existIf ? '[]})' : `[], "${_uuid}")`;
        }
      } else {
  ...
  function getVirtualDomCreator (template) {
    let result = 'function render(_, h, state) {\n';
    indent++;
